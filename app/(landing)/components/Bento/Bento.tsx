	...
	return (
		<section className={`${styles.bento} bento`} ref={ref}>
			<div className={styles.heading}>
				...
				<h2>Un laboratoire quantique interactif, pensé pour apprendre en manipulant.</h2>
			</div>

			<div className={`grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-4 h-full ${styles.grid}`}>
				<Card
					className={`md:col-span-2 md:row-span-2 text-white flex flex-col justify-end overflow-hidden border-none ${styles.waveCard}`}
				>
					<CardHeader>
				...
				<Card className={`text-white border-none ${styles.highlightCard}`}>
					...
					<CardContent>
						<p>
							Un package Python clé en main. Toutes les fonctions quantiques fondamentales
							encapsulées dans un package installable. Intégrez-les dans vos scripts, vos calculs,
							vos projets personnels en une ligne.
						</p>
					</CardContent>
				</Card>
				...
				<Card className={`border-none ${styles.softCardAlt}`}>
					...
					<CardContent>
						<p>
							Un projet open source dédié à l'éducation en mécanique quantique, accessible à tous.
						</p>
					</CardContent>
				</Card>
			</div>
